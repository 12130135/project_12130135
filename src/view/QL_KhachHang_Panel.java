...
public class QL_KhachHang_Panel extends JPanel implements ActionListener,
		MouseListener {
	...
	public QL_KhachHang_Panel() {
		...
		title_Panel = new TitlePanel("Quản Lý Khách Hàng");
		...
		bt_timkiem.setIcon(new ImageIcon(QL_KhachHang_Panel.class
				.getResource("/images/calenda/search.png")));
		panel_search.add(bt_timkiem);

		btn_capnhat = new JButton("Cập Nhật");
		btn_capnhat.setIcon(new ImageIcon(QL_KhachHang_Panel.class
				.getResource("/images/calenda/Refresh-icon.png")));
		panel_search.add(btn_capnhat);
		// End Phần Tìm kiếm ===========================================

		// JScrollPane scroll = new JScrollPane(table);
		JPanel panel_Show_KhachHang = new JPanel();
		panel_Show.add(panel_Show_KhachHang, BorderLayout.CENTER);
		table = new JTable();

		columnames.add(" ID KH ");
		columnames.add(" Tên Khách Hàng ");
		columnames.add(" Giới Tính ");
		columnames.add(" Địa Chỉ ");
		columnames.add(" Số Điện Thoại ");
		columnames.add(" Email ");
		columnames.add(" Ghi Chú ");
		...
		table.revalidate();
		panel_Show_KhachHang.add(scroll);
	...
	public QL_KhachHang_Panel(QL_KhachHang_Panel ql_KhachHang_Panel) {
	...
	// Dữ liệu lấy từ Panel_Button_AllTextField_KhachHang
	public static void loadDatabase() {
		loadDatabase_DAO();
	}

	private static void loadDatabase_DAO() {
		List<KhachHang> list = Controller_KhachHang.loadDatabase();
		record = null;
		datamodel = new DefaultTableModel(record, columnames);
		table.setModel(datamodel);
		...
		for (KhachHang KhachHang : list) {
			record = new Vector();
			record.addElement(KhachHang.getMaKhachHang());
			record.addElement(KhachHang.getTenKhachHang());
			record.addElement(KhachHang.getGioiTinh());
			record.addElement(KhachHang.getDiaChi());
			record.addElement(KhachHang.getSoDienThoai());
			record.addElement(KhachHang.getEmail());
			record.addElement(KhachHang.getGhiChu());
	...
	public static void Export_Excell() {
		Export_File_Excell.fillData(table, new File("D:\\ListKhachHang.xls"));
	}

	public static void themKhachHang(String maKH, String tenKhachHang,
			String gioiTinh, String diaChi, String soDienThoai, String email,
			String ghiChu) {

		Controller_KhachHang.themKH(maKH, tenKhachHang, gioiTinh, diaChi,
				soDienThoai, email, ghiChu);

		record = new Vector();
		record.addElement(maKH);
		record.addElement(tenKhachHang);
		record.addElement(gioiTinh);
		record.addElement(diaChi);
		record.addElement(soDienThoai);
		record.addElement(email);
		record.addElement(ghiChu);
	...
	private void timkiem(String serch) {
		List<KhachHang> list = Controller_KhachHang.timKiem(serch);
		record = null;
		datamodel = new DefaultTableModel(record, columnames);
		table.setModel(datamodel);
	...
	public void mouseClicked(MouseEvent o) {
		if (o.getSource().equals(QL_KhachHang_Panel.getTable())) {
			table = QL_KhachHang_Panel.getTable();

			int row = table.getSelectedRow();

			Panel_Button_AllTextField_KhachHang.getTf_MaKhachHang().setText(
					(String) datamodel.getValueAt(row, 0));
			Panel_Button_AllTextField_KhachHang.getTf_TenKhachHang().setText(
					(String) datamodel.getValueAt(row, 1));
			Panel_Button_AllTextField_KhachHang.getListGioiTinh()
					.setSelectedItem(table.getModel().getValueAt(row, 2));
			Panel_Button_AllTextField_KhachHang.getTf_DiaChi().setText(
					(String) datamodel.getValueAt(row, 3));
			Panel_Button_AllTextField_KhachHang.getTf_SoDienThoai().setText(
					(String) datamodel.getValueAt(row, 4));
			Panel_Button_AllTextField_KhachHang.getTf_Email().setText(
					(String) datamodel.getValueAt(row, 5));
			Panel_Button_AllTextField_KhachHang.getTf_GhiChu().setText(
					(String) datamodel.getValueAt(row, 6));

			Panel_Button_AllTextField_KhachHang.getTf_MaKhachHang()
					.setEditable(false);
			return;
		}
	}
